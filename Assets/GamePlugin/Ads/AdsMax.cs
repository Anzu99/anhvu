...
namespace mygame.sdk
{
    public class AdsMax : AdsBase
    {
        ...
        protected override void tryLoadBanner(int type)
        {
            ...
            MaxSdkBase.BannerPosition adposbn;
            if (dicBanner[type].posBanner == 0)
            {
                adposbn = MaxSdkBase.BannerPosition.TopCenter;
            }
            else if (dicBanner[type].posBanner == 1)
            {
                adposbn = MaxSdkBase.BannerPosition.BottomCenter;
            }
            else
            {
                adposbn = MaxSdkBase.BannerPosition.BottomCenter;
            ...
            MaxSdk.CreateBanner(bannerId, adposbn);
            isBnCreate = true;
            if (bnWidth > 0)
            {
            ...
            {
                SdkUtil.logd($"ads max{adsType} tryLoadBanner show");
                if (bnWidth > 0)
                {
                    MaxSdk.SetBannerWidth(bannerId, bnWidth);
                }
                ...
                {
                    MaxSdk.UpdateBannerPosition(bannerId, adposbn);
                    MaxSdk.ShowBanner(bannerId);
                }
            }
            else
            {
                SdkUtil.logd($"ads max{adsType} tryLoadBanner hide");
        ...
        protected override void tryLoadFull(bool _isSplash)
        {
            ...
            if (_isSplash)
            {
                MaxSdk.LoadInterstitial(splashId);
            }
            else
            {
                MaxSdk.LoadInterstitial(fullId);
            }
#else
            if (cbFullLoad != null)
            {
