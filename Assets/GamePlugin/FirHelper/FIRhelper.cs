    ...
    public class FIRhelper : MonoBehaviour
    {
        ...
        public static void logEvent(string title)
        {
            ...
            else
            {
                ...
                mem.nameevent = title;
                listWaitLog.Add(mem);
                Debug.Log("mysdk: firhelper memlog=" + title);
            }
#endif
        ...
        public static void logEvent(string name, Dictionary<string, object> dicParams)
        {
            ...
            else
            {
                ...
                listWaitLog.Add(mem);
                Debug.Log("mysdk: firhelper memlog=" + name);
            }
#endif
        ...
        private void parserCfGame()
        {
            ...
            v = FirebaseRemoteConfig.DefaultInstance.GetValue("cf_admix_CMP");
            if (v.StringValue != null && v.StringValue.Length > 0)
            {
                int per = (int)v.LongValue;
                PlayerPrefs.SetInt("cf_admix_CMP", per);
            }
            v = FirebaseRemoteConfig.DefaultInstance.GetValue("cf_admix_enable");
            if (v.StringValue != null && v.StringValue.Length > 0)
            {
                int per = (int)v.LongValue;
                PlayerPrefs.SetInt("cf_admix_enable", per);
            }
            v = FirebaseRemoteConfig.DefaultInstance.GetValue("cf_admix_alshow");
            if (v.StringValue != null && v.StringValue.Length > 0)
            {
                int per = (int)v.LongValue;
                PlayerPrefs.SetInt("cf_admix_alshow", per);
            }
            v = FirebaseRemoteConfig.DefaultInstance.GetValue("cf_admix_click");
            if (v.StringValue != null && v.StringValue.Length > 0)
            {
                int per = (int)v.LongValue;
                PlayerPrefs.SetInt("cf_admix_click", per);
            }
...
#if ENABLE_ADMIX
#if UNITY_IOS || UNITY_IPHONE
            string keyadmix = "admix_df_v1_ios";
#else
            string keyadmix = "admix_df_v1_android";
#endif
            v = FirebaseRemoteConfig.DefaultInstance.GetValue(keyadmix);
            if (v.StringValue != null && v.StringValue.Length > 0)
            {
                var admixdefault = (IDictionary<string, object>)JsonDecoder.DecodeText(v.StringValue);
                if (admixdefault != null || admixdefault.Count > 0)
                {
                    if (admixdefault.ContainsKey("6x5"))
                    {
                        List<object> listgameAdmix = (List<object>)admixdefault["6x5"];
                        string dataadmixdf = "";
                        for (int kk = 0; kk < listgameAdmix.Count; kk++)
                        {
                            var admixgame = (IDictionary<string, object>)listgameAdmix[kk];
                            string img = (string)admixgame["img"];
                            string link = (string)admixgame["link"];
                            if (dataadmixdf.Length > 0)
                            {
                                dataadmixdf += "#";
                            }
                            dataadmixdf += img;
                        ...
                        Debug.Log($"mysdk: admix get cf5=" + dataadmixdf);
                        PlayerPrefs.SetString("admix_v1_6x5", dataadmixdf);
                    }
                    // AdmixHelper.configDefault();
                    GadsmeHelper.configDefault();
                }
            }
#endif//ENABLE_ADMIX
