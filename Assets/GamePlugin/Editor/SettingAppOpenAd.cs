...
public class SettingBuildAndroid : Editor
{
    ...
    string GameName = "";
    string MemGameName = "";
    public static bool isAdmix = false;
    ...
    private string getMemSetting(string key, string defaultvalue)
    {
    ...
    private void setMemSetting(string key, string value)
    {
        string levelDirectoryPathSrc = Application.dataPath + "/GamePlugin/Editor/.memsetting.txt";
        try
        {
    ...
    void addValidIapAndroid(string bkey)
    {
        try
        {
            if (forceGenBaseKey)
            {
                ...
                if (bkey != null && bkey.Length > 50 && bkey.StartsWith("MIIBI") && bkey.EndsWith("AQAB"))
                {
                    byte[] makey = new byte[bkey.Length];
                    int nl = UnityEngine.Random.Range(10, 20);
                    int[] paskey = new int[nl];
                    byte[] pasva = new byte[nl];
                    int nd = bkey.Length / (nl + 21);
                    int bran = nd / 3;
                    for (int i = 0; i < nl; i++)
                    {
                        paskey[i] = nd * (i + 1) + UnityEngine.Random.Range(-bran, bran);
...
public class SdkSetup
{
        ...
        try
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                if (lines[i].Contains("#import <GoogleMobileAds/GoogleMobileAds.h>"))
                {
                    allline.Add(lines[i]);
                    int nl = i + 1;
                    if (SettingBuildAndroid.isAdmix)
                    {
    ...
    private static void doAddGradle(mygame.sdk.AppType typeapp, bool isAddAndroidGms, mygame.sdk.AdsHelper groupControl)
    {
        ...
        try
        {
            ...
            if (statusAddAds != 1 || statusAddIdentifier != 1 || !ishaslifecycle || statusAndroidsupport != 0 || statusAddPlaycore == 0 || (statusAddNativeCmp == 0 && SettingBuildAndroid.isAdmix || statusAddNativeCmp == 1 && !SettingBuildAndroid.isAdmix) || (statusAddAppset4Iron == 0 && SettingBuildAndroid.isIronEnable))
            {
                int isAdd = 0;
                List<string> allline = new List<string>();
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    if (isAdd == 1)
                    {
                        ...
                        else if (lines[i].StartsWith("**DEPS**}") || lines[i].StartsWith("}") || lines[i].Contains("// Android Resolver Dependencies End"))
                        {
                            ...
                            if (statusAddNativeCmp == 0 && SettingBuildAndroid.isAdmix)
                            {
